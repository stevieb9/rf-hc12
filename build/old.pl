...
my $fd = tty_open('/dev/ttyUSB0', 9600);

say tty_puts($fd, 'AT');

#int tty_read_until(int fd, char* buf, char until){
...
char *tty_read_until(int fd, char* buf, char until){
    char b[1];
    int i=0;
    do {
        int n = read(fd, b, 1);  // read a char at a time
        if( n==-1){
            croak("failed to read from serial port");
        }
        if( n==0 ) {
